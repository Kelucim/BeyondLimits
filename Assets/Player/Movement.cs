...
public class Movement : MonoBehaviour
{
    ...
    {   
        ...
        if (gravityAction.triggered && isGroundedGOn && GameManager.gameStatus == 1)
        {
            gravityOn = !gravityOn;
        }
        else if(gravityAction.triggered && isGroundedGOff && GameManager.gameStatus == 1)
        {
            gravityOn = !gravityOn;
        }

        // Apply the appropriate force of gravity
        if (gravityOn)
        {
        ...
        if(!isGroundedGOff && !isGroundedGOn)
        {
            PlAnim.SetBool("IsFalling", true);
        }
        else
        {
            PlAnim.SetBool("IsFalling", false);
        }
    }
}
